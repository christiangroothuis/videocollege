...
const sfapikey = import.meta.env.VITE_APP_SFAPIKEY;
...
): { data: Home; isLoading: boolean; isError: boolean } => {
...
const folderIdsToQuery = (...folderIds: string[]) =>
    folderIds
        .map((folderId: string) => `ParentFolderId:${folderId}`)
        .join(" OR ");
